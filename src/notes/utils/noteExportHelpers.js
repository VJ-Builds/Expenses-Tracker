...
export const exportNoteAsTxt = async (note) => {
  try {
    ...
    if (hasText) {
      textContent += `${safeNote.content.trim()}\n\n`;
    }
    ...
    if (hasChecklist) {
      ...
      safeNote.checklist_data.forEach((item) => {
    ...
    // 1. Direct download to chosen folder (Downloads) via Storage Access Framework
    const savedDir = await saveFileToDirectory(fileName, 'text/plain', textContent);
    if (savedDir) {
      Alert.alert(
        'Download Complete 📥',
...
export const exportNoteAsPdf = async (note) => {
  try {
    ...
    try {
      ...
      if (hasText) {
        const formattedContent = escapeHtml(safeNote.content.trim()).replace(/\n/g, '<br/>');
        bodyHtml += `<div class="note-body">${formattedContent}</div>`;
      }
      ...
      const html = `
        ...
        <head>
          ...
          <style>
            body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #1C1C28; background: #FFFFFF; padding: 36px; margin: 0; line-height: 1.6; }
            .header { border-bottom: 2px solid #F0F1F5; padding-bottom: 18px; margin-bottom: 24px; }
            .folder-badge { display: inline-block; background: #EEF2FF; color: #2563EB; font-size: 11px; font-weight: 700; padding: 4px 12px; border-radius: 999px; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 10px; }
            h1 { font-size: 24px; font-weight: 800; color: #0F172A; margin: 0 0 8px 0; }
            .meta { font-size: 12px; color: #64748B; }
            .tags-wrap { margin-top: 8px; }
            .tag { display: inline-block; background: #F1F5F9; color: #475569; font-size: 11px; font-weight: 600; padding: 2px 8px; border-radius: 6px; margin-right: 6px; }
            .note-body { font-size: 15px; color: #334155; white-space: pre-wrap; line-height: 1.7; }
            .section-divider { margin: 24px 0 16px 0; border-top: 1px dashed #CBD5E1; }
            .section-title { font-size: 13px; font-weight: 700; color: #475569; text-transform: uppercase; letter-spacing: 0.6px; margin: 0 0 12px 0; }
            .checklist { display: flex; flex-direction: column; gap: 10px; }
            .check-item { display: flex; align-items: center; gap: 12px; font-size: 15px; color: #1E293B; }
            .check-item.completed .text { text-decoration: line-through; color: #94A3B8; }
            ...
            .footer { margin-top: 48px; padding-top: 16px; border-top: 1px solid #F1F5F9; font-size: 11px; color: #94A3B8; text-align: center; }
          </style>
        </head>
        <body>
          <div class="header">
...
const buildPureJsPdf = (note, dateStr) => {
  ...
  if (hasText) {
    bodyLines.push(...wrapText(note.content.trim(), 72));
  }
  ...
  if (hasChecklist) {
    ...
    note.checklist_data.forEach((item) => {
